...
%Need to get companys (column indexs) that have valid returns for last window days
    validCovStocks = coVarIndex_Grp7(returnSubSet);
%Need to have valid companies on this trading day (i)
    validCompany = validIndex_Grp7(stdmom(i,:),live(i,:),r(i+1,:),mv(i,:));
...
[shrinkMatrix,shrinkFactor(k)] = shrinkCoVar_Grp7(returnSubSet(:,validIndex));
...
riskAdjGammaThree = volScaling_Grp7(gammaThree(1,:),0.03,250);
 
%Regression Statisitics
gammaStats_Grp7(gammaThree(1,:),riskAdjGammaThree)

%Plot results
plotGamma_Grp7(gammaThree(1,:),riskAdjGammaThree,3)
